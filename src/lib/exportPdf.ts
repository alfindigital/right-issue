...
export async function generateRightIssuePDF(
  data: ExportPDFData,
  ...
  if (variant === 'compact') {
    ...
    doc.setTextColor(...colors.muted);
    doc.setFontSize(7);
    doc.text(`Right Issue Calculator • lotmetrik.my.id • ${timestamp}`, pageWidth / 2, 290, { align: 'center' });
  ...
  doc.setTextColor(...colors.muted);
  doc.setFontSize(7);
  doc.text(`Generated by Right Issue Calculator • lotmetrik.my.id • ${timestamp}`, pageWidth / 2, 290, { align: 'center' });
